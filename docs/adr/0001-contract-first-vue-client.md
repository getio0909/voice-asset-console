...
Server contract `0.1.0`, checks `/api/v1/system/capabilities` at startup, and
...
pin. Product views remain explicitly incomplete until a tested Server vertical
slice exists. Future OpenAPI generation must preserve the boundary and update the
compatibility matrix with the contract source commit.
