...
test('pins multi-platform base images and declares immutable OCI labels', () => {
  assert.match(dockerfile, /FROM node:24\.12\.0-alpine3\.23@sha256:[0-9a-f]{64} AS build/)
  assert.match(dockerfile, /FROM caddy:2\.10\.2-alpine@sha256:[0-9a-f]{64} AS runtime/)
...
test('checksums and release verification cover the OCI archive', () => {
  ...
  assert.match(
    ci,
    /node --test scripts\/verify-oci-image\.node-test\.mjs scripts\/container-release\.node-test\.mjs/,
  )
})
