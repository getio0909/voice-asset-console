    ...
    while (position + blockSize <= stat.size) {
      ...
      if (type === '5') {
        ...
        const allowed =
          name === 'oci-layout' ||
          name === 'index.json' ||
          /^blobs\/sha256\/[0-9a-f]{64}$/.test(name)
...
function validateImage(metadata, blobSizes, descriptor, expected) {
  ...
  if (config.config?.User !== '65532:65532')
    fail(`image must run as non-root user 65532:65532 for ${platformName}`)
...
if (index.schemaVersion !== 2 || !Array.isArray(index.manifests))
  fail('index.json is not an OCI image index')
const expected = {
  image,
  version,
  commit,
...
if (
  expected.seen.size !== 2 ||
  [...expected.platforms].some((platform) => !expected.seen.has(platform))
) {
